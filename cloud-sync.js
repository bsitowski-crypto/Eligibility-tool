(function(){
  ...
  function installCloudUI(){
    ...
    style.textContent=`
#cloudAuthGate,#cloudAdminModal{position:fixed;inset:0;z-index:1000;background:#071f3eee;display:flex;align-items:center;justify-content:center;padding:18px;font-family:-apple-system,BlinkMacSystemFont,"Helvetica Neue",Arial,sans-serif}
#cloudAuthGate.hidden,#cloudAdminModal.hidden{display:none!important}.cloud-auth-box,.cloud-admin-box{width:min(470px,100%);max-height:88vh;overflow:auto;background:#fff;border-radius:18px;padding:22px;box-shadow:0 18px 55px #0006;color:#1c1c1e;box-sizing:border-box}.cloud-auth-box h2,.cloud-admin-box h2{font-size:24px;color:#071f3e;margin:0 0 6px}.cloud-auth-box p,.cloud-admin-box p{margin:0 0 16px;color:#666;line-height:1.45}.cloud-auth-box label,.cloud-admin-box label{display:block;font-weight:700;margin:0 0 6px}.cloud-auth-box input,.cloud-admin-box input{font:inherit;font-size:16px;width:100%;box-sizing:border-box;padding:11px;border:1px solid #d6d6dc;border-radius:10px;background:#fff;margin-bottom:13px}.cloud-auth-box button,.cloud-admin-box button{font:inherit;width:100%;border:0;border-radius:10px;padding:12px 14px;background:#0b63ce;color:#fff;font-weight:800}.cloud-auth-box button:disabled,.cloud-admin-box button:disabled{opacity:.55}.cloud-auth-error{display:none;background:#fff0f0;color:#8a1d1d;border-left:5px solid #c93535;padding:10px;border-radius:8px;margin:10px 0;font-size:13px}.cloud-auth-note{font-size:12px;color:#666;line-height:1.45;margin-top:12px}.cloud-status{display:inline-block;font-size:11px;font-weight:800;padding:5px 8px;border-radius:999px;background:#a86b00;color:#fff;white-space:nowrap;margin-right:6px}.cloud-status.ok{background:#2e8b57}.cloud-status.bad{background:#a52b2b}#cloudSignOutBtn,#cloudAdminBtn{display:none}.cloud-admin-row{border-top:1px solid #e5e5ea;padding:12px 0;display:flex;gap:10px;align-items:center}.cloud-admin-row .who{flex:1;min-width:0}.cloud-admin-row .email{font-weight:800;overflow-wrap:anywhere}.cloud-admin-row .uid{font-size:11px;color:#777;overflow-wrap:anywhere}.cloud-admin-row button{width:auto;min-width:90px;background:#a52b2b;padding:9px 10px}.cloud-admin-actions{display:flex;gap:8px;margin-top:10px}.cloud-admin-actions button{flex:1}.cloud-admin-actions .secondary{background:#68707a}.cloud-admin-success{display:none;background:#eefaf2;color:#23633a;border-left:5px solid #2e8b57;padding:10px;border-radius:8px;margin:10px 0;font-size:13px}
`;
    document.head.appendChild(style);

    const gate=document.createElement("div");
    gate.id="cloudAuthGate";
    gate.innerHTML=`<div class="cloud-auth-box"><h2>Solvita Planner Sign In</h2><p>Sign in with an authorized Solvita Planner account.</p><label>Email</label><input id="cloudEmail" type="email" inputmode="email" autocomplete="username" placeholder="name@example.com"><label>Password</label><input id="cloudPassword" type="password" autocomplete="current-password" placeholder="Password"><div id="cloudAuthError" class="cloud-auth-error"></div><button id="cloudSignInBtn" type="button">SIGN IN</button><div class="cloud-auth-note">Accounts are created and approved by the planner administrator.</div></div>`;
    document.body.appendChild(gate);

    const admin=document.createElement("div");
    admin.id="cloudAdminModal";admin.className="hidden";
    admin.innerHTML=`<div class="cloud-admin-box"><h2>User Administration</h2><p>Create an approved planner account. The new user can sign in immediately with the email and temporary password you set here.</p><label>Email</label><input id="adminNewEmail" type="email" inputmode="email" placeholder="coworker@example.com"><label>Temporary password</label><input id="adminNewPassword" type="password" autocomplete="new-password" placeholder="At least 6 characters"><div id="adminError" class="cloud-auth-error"></div><div id="adminSuccess" class="cloud-admin-success"></div><button id="adminCreateBtn" type="button">CREATE & APPROVE USER</button><div class="cloud-admin-actions"><button id="adminRefreshBtn" class="secondary" type="button">REFRESH LIST</button><button id="adminCloseBtn" class="secondary" type="button">CLOSE</button></div><h3>Approved users</h3><div id="adminUserList"><div class="cloud-auth-note">Loading…</div></div><div class="cloud-auth-note">Revoking access removes the user from the planner allow-list but does not delete their Firebase Authentication account.</div></div>`;
    ...
    if(ver)ver.textContent="Version 9.10.3 — Firebase Login + Admin User Management";
  ...
  async function renderApprovedUsers(){
    ...
    try{
      ...
      snap.forEach(doc=>{
        ...
        row.innerHTML=`<div class="who"><div class="email"></div><div class="uid"></div></div><button type="button">REVOKE</button>`;
        row.querySelector(".email").textContent=d.email||"(no email)";row.querySelector(".uid").textContent=doc.id;
        row.querySelector("button").addEventListener("click",()=>adminRevokeUser(doc.id,d.email||"this user"));list.appendChild(row);
      });
  ...
  async function adminCreateUser(){
    ...
    try{
      ...
      await cloudDb.collection("approvedUsers").doc(uid).set({email,approved:true,createdAt:firebase.firestore.FieldValue.serverTimestamp(),createdBy:cloudAuth.currentUser.email||ADMIN_EMAIL});
      document.getElementById("adminNewEmail").value="";document.getElementById("adminNewPassword").value="";
      ok.textContent="User created and approved: "+email;ok.style.display="block";await renderApprovedUsers();
