(function(){
  ...
  function refreshFilterUI(){
    const p=document.getElementById("f_pending"),a=document.getElementById("f_planning"),ip=document.getElementById("f_in_progress"),c=document.getElementById("f_complete");
  ...
  function install(){if(installed)return true;if(typeof window.renderBoard!=="function"||!document.getElementById("status"))return false;migrate();try{filters.active=filters.active!==false}catch{};simplifySelect();window.renderBoard=renderSimpleBoard;replaceRestore();replaceUpdateStatus();
  ...
  setInterval(()=>{if(!installed)return;simplifySelect();refreshFilterUI()},1500);
})();
