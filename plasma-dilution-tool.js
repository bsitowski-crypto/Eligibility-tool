(function(){
  ...
  const groups={
    ...
    crystalloid:["saline","ringers_lactate","dextrose","balanced_electrolyte","tpn","medication","other_crystalloid"]
  };
...
#plasmaDilutionTool .pd-fields{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:8px;padding:10px 12px}.pd-span-2{grid-column:span 2}.pd-span-all{grid-column:1/-1}#plasmaDilutionTool label{display:block;font-size:12px;font-weight:700;color:#42536b}#plasmaDilutionTool input,#plasmaDilutionTool select{display:block;width:100%;height:38px;margin-top:4px;border:1px solid #aebdd0;border-radius:7px;background:#fff;color:#172a44;padding:7px 8px;font:inherit;font-size:16px;font-variant-numeric:tabular-nums}#plasmaDilutionTool input[aria-invalid=true],#plasmaDilutionTool select[aria-invalid=true]{border:2px solid #a32a3e}.pd-unit{display:grid;grid-template-columns:minmax(0,1fr) 120px;gap:6px}.pd-conversion{display:block;margin-top:4px;color:#5c6d83;font-size:11px;font-weight:500}.pd-window{display:flex;justify-content:space-between;gap:8px;padding:8px 10px;background:#eaf1f9;border-radius:7px;font-size:13px}.pd-window strong{font-variant-numeric:tabular-nums}
...
@media(max-width:650px){#plasmaDilutionTool{width:calc(100vw - 12px);max-height:calc(100dvh - 12px)}#plasmaDilutionTool .pd-head{padding:10px 12px}#plasmaDilutionTool h2{font-size:18px}#plasmaDilutionTool .pd-head span{display:none}#plasmaDilutionTool .pd-caution{margin:8px 9px 0}#plasmaDilutionTool .pd-body{padding:8px 9px 12px}#plasmaDilutionTool .pd-fields{grid-template-columns:1fr 1fr}.pd-span-2{grid-column:span 2}.pd-flags{grid-template-columns:1fr}.pd-fluid-line{grid-template-columns:minmax(0,1fr) 78px 36px}.pd-fluid-line label:nth-child(2){grid-column:1/2}.pd-fluid-line label:nth-child(3){grid-column:2/3}.pd-fluid-line label:nth-child(4){grid-column:1/3}.pd-fluid-line button{grid-column:3;grid-row:1}.pd-incomplete-fields{grid-template-columns:1fr 1fr}.pd-incomplete-fields label:last-child{grid-column:1/-1}.pd-window{display:block}.pd-window strong{display:block;margin-top:3px}}
...
`;
      ...
      <div class="pd-body"><div class="pd-layout"><main class="pd-main">
        <section class="pd-box"><div class="pd-title"><span class="pd-step">1</span><h3>Donor and sample</h3></div>
          <div class="pd-fields">
            ...
            <label class="pd-span-2">Height<div class="pd-unit"><input id="pd-height" inputmode="decimal"><select id="pd-height-unit"><option value="in">Inches</option><option value="cm">Centimeters</option></select></div><span class="pd-conversion" id="pd-height-conversion">Enter height</span></label>
            <label class="pd-span-2">Weight<div class="pd-unit"><input id="pd-weight" inputmode="decimal"><select id="pd-weight-unit"><option value="lb">Pounds</option><option value="kg">Kilograms</option></select></div><span class="pd-conversion" id="pd-weight-conversion">Enter weight</span></label>
            <label>Sample draw date<input id="pd-sample-date" type="date"></label><label>Sample draw time<input id="pd-sample-time" inputmode="numeric" maxlength="5" placeholder="HHMM"></label>
            <label>Cardiac asystole date<input id="pd-asystole-date" type="date"></label><label>Cardiac asystole time<input id="pd-asystole-time" inputmode="numeric" maxlength="5" placeholder="HHMM"></label>
  ...
  function addFluid(category,focus=true){
    ...
    row.innerHTML=`<div class="pd-fluid-line"><label>Component<select class="pd-component">${componentOptions(category)}</select></label><label>Start date<input class="pd-entry-date" type="date" value="${today()}"></label><label>Time<input class="pd-entry-time" inputmode="numeric" maxlength="5" placeholder="HHMM"></label><label>Volume (mL)<input class="pd-entry-volume" inputmode="decimal"></label><button type="button" class="pd-remove" aria-label="Remove fluid">×</button></div><label class="pd-check pd-incomplete-toggle"><input class="pd-incomplete" type="checkbox"> Volume unknown or infusion incomplete at death</label><div class="pd-incomplete-fields" hidden><label>Access<select class="pd-access"><option value="iv-22">22g IV · 35 mL/min</option><option value="iv-20">20g IV · 65 mL/min</option><option value="iv-18" selected>18g IV · 105 mL/min</option><option value="iv-16">16g IV · 220 mL/min</option><option value="io-tibia">Tibial IO · 31 mL/min</option><option value="io-humerus">Humeral IO · 57 mL/min</option><option value="io-sternum">Sternal IO · 94 mL/min</option></select></label><label>End date<input class="pd-entry-end-date" type="date" value="${today()}"></label><label>End time<input class="pd-entry-end-time" inputmode="numeric" maxlength="5" placeholder="HHMM"></label><label>Bag volume (mL)<input class="pd-bag-volume" inputmode="decimal" value="1000"></label><label>Calculated maximum<input class="pd-calculated" readonly value="—"></label></div><div class="pd-entry-note">Enter a documented volume or use worst-case mode.</div>`;
    const component=row.querySelector(".pd-component"),volume=row.querySelector(".pd-entry-volume");
    function applyDefault(){const item=engine.COMPONENTS[component.value];volume.value=item?.volume??"";update();}
    ...
    row.querySelector(".pd-incomplete").addEventListener("change",event=>{row.querySelector(".pd-incomplete-fields").hidden=!event.target.checked;volume.disabled=event.target.checked;update();});
  ...
  function collect(){
    ...
    return {
      donorId:value("#pd-donor-id"),age:value("#pd-age"),sex:value("#pd-sex"),height:value("#pd-height"),heightUnit:value("#pd-height-unit"),weight:value("#pd-weight"),weightUnit:value("#pd-weight-unit"),
  ...
  function update(){
    ...
    dialog.querySelector("#pd-height-conversion").textContent=m.heightInches?data.heightUnit==="cm"?`Automatically converted: ${m.heightInches.toFixed(2)} in`:`Automatically converted: ${m.heightCentimeters.toFixed(2)} cm`:"Enter height";
    dialog.querySelector("#pd-weight-conversion").textContent=m.weightPounds?data.weightUnit==="kg"?`Automatically converted: ${m.weightPounds.toFixed(2)} lb`:`Automatically converted: ${m.weightKilograms.toFixed(2)} kg`:"Enter weight";
  ...
  function clear(){
    ...
    dialog.querySelector("#pd-blood-loss").value="no";dialog.querySelector("#pd-height-unit").value="in";dialog.querySelector("#pd-weight-unit").value="lb";
