...
test("approved access requires an active approval and completed password change",()=>{
  const start=rules.indexOf("function isApproved()"),end=rules.indexOf("function teamBoardValid",start),block=rules.slice(start,end);
...
test("new team items cannot forge authorship or initial history",()=>{
  const start=rules.indexOf("allow create: if isApproved()",rules.indexOf("match /teamBoardItems")),end=rules.indexOf("allow update:",start),block=rules.slice(start,end);
