...
function openView(){ensure();view.classList.remove("hidden");render()}
function openForm(id){editingId=id||null;const d=id?load().find(x=>String(x.id)===String(id))||{}:{};for(const [k,v] of [["meName",d.name],["mePhone",d.phone],["meMorgue",d.morgue],["meNotes",d.notes]])view.querySelector("#"+k).value=v||"";view.querySelector("#meForm").classList.add("open");view.querySelector("#meAdd").style.display="none"}
function closeForm(){if(!view)return;editingId=null;view.querySelector("#meForm").classList.remove("open");view.querySelector("#meAdd").style.display="block"}
function saveForm(){const name=view.querySelector("#meName").value.trim();if(!name){alert("Enter the medical examiner name.");return}const a=load(),data={name,phone:view.querySelector("#mePhone").value.trim(),morgue:view.querySelector("#meMorgue").value.trim(),notes:view.querySelector("#meNotes").value.trim()};if(editingId){const i=a.findIndex(x=>String(x.id)===String(editingId));if(i>=0)a[i]={...a[i],...data}}else a.push({id:"me_"+Date.now().toString(36),...data});store(a);closeForm();render()}
